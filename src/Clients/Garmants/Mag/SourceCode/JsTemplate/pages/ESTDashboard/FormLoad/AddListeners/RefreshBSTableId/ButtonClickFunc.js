...
function jFLocalInsertItemCount(inData) {
    return Promise.all([
        ...
        const processedData = inData.map(item => {
            const filteredData1 = fetchData1.filter(fetchItem => fetchItem.RefPK === item.pk.toString());
            const filteredData2 = fetchData2.filter(fetchItem => fetchItem.RefPK === item.pk.toString());
